...
owner glances at. The card border is gone as of the revision below; the rest still holds.

## Revision — paper soft, and hero first

The original was rejected a second time, in plainer terms: boring, and unclear what the text was
for. Three rounds of exploration answered two different questions, and it is worth recording that
they were different, because the first round answered the wrong one.

- **Fourteen navigation structures** — wireframes, all rejected.
  <https://claude.ai/code/artifact/df72be70-5cb7-4840-9a7f-5b7835e636f6>
- **Eighteen looks** — the same screen in eighteen palettes and styles, which is where Soft UI was
  picked. <https://claude.ai/code/artifact/3b15c17d-267c-4c49-b350-3afbabb70f50>
- **Ten readings of Soft UI**, with the dashboard and its charts. 09 (paper soft) and 10 (glass
  navigation bar) were picked together.
  <https://claude.ai/code/artifact/78ce34b8-a41f-4ccc-90e2-7e2b4b7b8a47>
- **Thirteen layouts in that one frozen look**, where 01 (hero number first) was chosen.
  <https://claude.ai/code/artifact/e149b89a-16e3-416a-b400-21090d22a797>

**What changed: a card is raised, not outlined.** The 1px border is replaced by two shadow layers
(`--elev-1`, `--elev-2`, together as `shadow-card`), and the ground moved from `#fafafa` to
`#f4f5f7` to give those shadows something to fall on. The accent moved from `#148b92` to `#3f7d76`
and `--muted-foreground` was darkened to `#697280` — the second is the one that mattered, because
the inherited shadcn value measured about 3.1:1 on white and was carrying real labels.

**What was rejected, and this is the part to keep.** True Soft UI makes the card and the page the
_same_ colour, so the card extrudes rather than sits. Measured across ten variants of that idea,
seven failed the small-text floor and six failed the chart-hue floor — not ten unlucky palettes,
the aesthetic itself: a grey surface has less contrast to spend than white, and Spa green drops
from 3.42:1 on white to 3.00:1 on `#eef0f3`. So this keeps the white card and takes only the
softness of the edge. If somebody later "finishes" the Soft UI look by matching the card to the
ground, that is the regression, and `tests/palette.test.ts` is what should catch it.

**Nesting.** A raised element inside a raised element has two elevations at once and reads as a
rendering fault. Branch rows are cards; the business name above them is a label on the ground, not
a card containing them.
...
| Token                                           | Value                             | Job                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------------------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--background`                                  | `#f4f5f7`                         | The page ground. Cooler and a step darker than the `#fafafa` it started at, because a card is now separated by elevation rather than by a line — a soft shadow needs a ground with enough tone to fall on, and on `#fafafa` it is invisible so the card floats on nothing.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `--card`                                        | `#ffffff`                         | The raised surface. 14px radius, and raised literally now: `shadow-card`, no border. Kept white rather than matched to the ground — see the elevation row.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `--elev-1` / `--elev-2`                         | see `globals.css`                 | The two shadow layers a card carries, exposed together as the `shadow-card` utility. Two rather than one because a single shadow reads as either a hard edge or a smudge: the tight layer does the border's old job and the wide one is light falling away. Dark restates both at far higher alpha, because a shadow is the absence of light and there is almost none to remove from a near-black ground — the light values land near 1.1:1 there and do not render at all.                                                                                                                                                                                                                                                                                                                                                                |
| `--commit`                                      | `#3f7d76`                         | Hue 186, chroma 0.065. **Three places, all enumerable:** the Record sale button, the ring around the rail dot for the business you are currently in, and the border of the `Current` chip on the branch list. The dot itself is that business's chart hue — the accent answers "which one am I in", the hue answers "which one is this", and the chip is that same question one level down, at the branch. Nowhere else: not a link, not a chart series, not the focus ring. **Never as text:** on the `Current` chip it is the border and never the label, because that label is 11.5px sitting on the ground. On the fill it is a different question and a better answer than before: white on `#3f7d76` measures 4.77:1, so a button label clears small text, where the `#148b92` this replaced managed 4.09:1 and was large-text-only. |
