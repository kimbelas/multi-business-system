...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- |
| **CONFIRMED PRESENT.** **Laundry taken before payment.** `laundry_orders.transaction_id` is `not null`, so an order cannot exist until money has changed hands. | If pay-on-claim happens at all, this is a column change now versus a backfill on the busiest table later. And an unpaid order must contribute nothing to that day's expected cash, or the blind close reports a variance that is an artefact of the payment mode. | Yes — 0017      |
| **CONFIRMED ABSENT.** **Two shifts, two closes.** `unique (branch_id, close_date)` means a second close would have to reopen the first.                         | Reopening turns a submitted count into an editable one, which is the property the whole control depends on.                                                                                                                                                       | Yes — 0018      |
...
| **CONFIRMED PRESENT.** **No float / starting cash.** `expected_cash` is computed from transactions alone.                                                       | If the drawer starts with a float, every variance is wrong by that amount.                                                                                                                                                                                        | No              |
...
| ----------------------------------------- | -------------- | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------- |
| Order taken before payment (pay-on-claim) | Laundry        | **must add**                    | Happens sometimes or often. `laundry_orders.transaction_id` becomes nullable, and an unpaid order contributes nothing to that day's expected cash.                              | Owner, 2026-09-01 |
| Second close in one day (two shifts)      | All            | **deliberately dropped**        | One close per branch per day. `unique (branch_id, close_date)` stays as specified.                                                                                              | Owner, 2026-09-01 |
| Starting cash in the drawer (float)       | All            | **must add**                    | There is a float. `expected_cash` computed from transactions alone would make every variance wrong by that amount.                                                              | Owner, 2026-09-01 |
...
than from shadowing. That is legitimate for these three: whether laundry is ever taken before payment,
whether a branch counts twice a day and whether the drawer holds a float are facts the owner knows and
does not need to observe.

**It does not close card 0001, and the difference matters.** Sections 1 to 3 are still empty, and they
are the ones that find fields _nobody thought to ask about_ — which is the entire reason the card asks
for shadowing rather than an interview. The three answers above remove the three most expensive
unknowns; the residual risk is the unknown unknowns, and only looking at the actual records finds
those.
